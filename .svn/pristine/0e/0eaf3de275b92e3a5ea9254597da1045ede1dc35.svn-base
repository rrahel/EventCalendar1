...
<body class="sidebar-mini 	<c:out value="${background}" />">
	<div class="wrapper">
		...
		<!--  add or edit?  ----------------------------------------------------------- -->
			<c:choose>
				<c:when test="${not empty ugroup}">
					<c:set var="legend">Edit Group ${userGroup.groupID}</c:set>
					<c:set var="formAction">editGroup</c:set>
					<c:set var="submitButton">Change</c:set>
					<c:set var="readonly">readonly</c:set>
				</c:when>
				<c:otherwise>
					<c:set var="legend">New Group</c:set>
					<c:set var="formAction">addGroup</c:set>
			...
			<li class="dropdown messages-menu">
				...
				<li class="dropdown user user-menu">
					...
					data-toggle="dropdown"> <!-- The user image in the navbar--> <img
						src="${pageContext.request.contextPath}/profilImage" class="user-image"
				...
				<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
					...
					<fieldset>
						...
						<input class="form-control" id="inputID" type="hidden" name="id"
							${readonly} value="<c:out value="${ugroup.groupID}"/>">
					
						<! ----------------  Name ---------------- -->
						<div class="form-group">
							<label for="inputName" class="col-md-2 control-label">Name</label>
							<div class="col-md-10">
								<input class="form-control" id="inputName" type="text" required="true" title="Required field!" name="name"
									 value="<c:out value="${ugroup.name}"/>">
							</div>
						</div>
						...
						<c:if test="${not empty ugroup}">
						
							<label for="inputMembers"  class="col-md-2 control-label">Existing group members</label>
								<div class="col-md-10">
								<select id="inputMembers" multiple name="groupMembers" class="form-control" >
									<c:forEach items="${groupMembers}" var="gmember">
											<option><c:out value="${gmember.username}"/></option>
								 </c:forEach>
					...
					<%-- 			
								<c:forEach items="${groupMembers}" var="gMemb">  						 
									
								 	<tr>  
     								   <td align="center">  
         								   <input type="checkbox" name="groupMembers" value="<c:out value="${gMemb.username}"/>"/>  
      								  </td>
								 		<td align="left"><c:out value="${gMemb.username}"/></td>  
								     </tr>  
									</c:forEach> --%>
						</div>
						</c:if>
						...
						<div class="form-group">
							<label for="inputMembers" class="col-md-2 control-label">Select new members</label>
								<div class="col-md-10">
								 <c:forEach items="${userMembers}" var="members">  						 
									
								 	<tr>  
     								   <td align="center">  
         								   <input type="checkbox" name="userMembers" value="<c:out value="${members.username}"/>"/>  
         								   &nbsp;
      								  </td>
								 		<td align="left"><c:out value="${members.username}"/><br></td>  
								     </tr>  
									</c:forEach>
								</div>
						</div>
							...
							<div class="col-md-10 col-md-offset-2">
								...
								<c:if test="${not empty ugroup}">
									<button type="submit" name="submitButton" value="remove"
										class="btn btn-danger">Remove Group</button>
								</c:if>
								<a href="listGroups">
