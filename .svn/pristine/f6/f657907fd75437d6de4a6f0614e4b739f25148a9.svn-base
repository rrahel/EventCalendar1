...
<script>
	...
	$(document).ready(function() {
		...
		$('#calendar').fullCalendar({
			height: "auto",
			firstDay: <c:out value="${user.firstDay}" />, 
			weekends: <c:out value="${user.weekends}" />,
			weekNumbers: <c:out value="${user.weekNumbers}" />,
			...
			eventClick: function(calEvent, jsEvent, view) {
				window.open('changeEvent?id='+calEvent.id,'_self');
				return false;
					},
...
<!-- 											<small><i class="fa fa-clock-o"></i> 5 mins</small> -->
 ...
 <!-- Sidebar Menu -->
		<ul class="sidebar-menu">
			...
			<li class="active"><a href=<c:url value='calendar'/>><i
					class='fa fa-calendar'></i> <span>Calendar</span></a></li>
			<li><a href=<c:url value='listGroups'/>><i class='fa fa-th'></i> <span>Modify
						Groups</span></a></li>
		...
		<div class="content-wrapper">
			...
			<!-- Main content -->
			<section class="content"> <!-- Your Page Content Here --> <!--  Error message ----------------------------------------------------------- -->
